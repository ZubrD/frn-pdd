<?php
    ...
    $key = file_get_contents('c:/openserver/domains/localhost/roadcontrol/server/the_key2.txt');
     ...
     */
    $OAUTH2_CLIENT_ID = '531886572921-fvvknc0fgundh7iecq0kgnrr7snhu1r8';    //Данные проекта roadcontrol2
    $OAUTH2_CLIENT_SECRET = 'SwGAfo6QsEW4BUwwiHomkQ42';
...
/*    if (isset($_SESSION['token'])) {
    ...
    if ($client->getAccessToken()) {
        ...
        if($client->isAccessTokenExpired()) {
            ...
            file_put_contents('the_key2.txt', $client->getAccessToken());
        }
        ...
        try {
            ...
            $query_select_reserve = "SELECT * FROM road_case WHERE server_video = 'yes' LIMIT 6";// Резервирую за скриптом testupload 3 строки
            ...
            while ($row = mysqli_fetch_array($result_select_reserve)){
                ...
                $query_reserve = "UPDATE road_case SET server_video = '', upload2 = 'yes' WHERE newid = '$id_reserve'";
                mysqli_query($dbc, $query_reserve);
            };

            //$query = "SELECT * FROM road_case WHERE upload2 = 'yes'"; // Если server_video = yes, значит видео ещё не загружено на youtube

            $query = "SELECT * FROM road_case INNER JOIN regions USING (id_reg) INNER JOIN violations USING (id_violation)
              INNER JOIN towns USING (id_town) WHERE upload2 = 'yes' AND road_case.id_reg = towns.id_reg AND road_case.id_violation = violations.id_violation"; //Связываю с таблицей road_case таблицы regions и towns, ограничивая выбор в таблице towns значением из поля id_reg
            ...
            while ($row = mysqli_fetch_array($result)){
                ...
                $id = $row['newid'];
                $video_id = $status['id'];
                $query_for_update = "UPDATE road_case SET upload2 = '', videos = '$video_id', dataa = NOW() WHERE newid = '$id'";  //Обнуляю, зарезервированное под этот скрипт поле upload2
